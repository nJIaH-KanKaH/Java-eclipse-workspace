...
public class Utils {
    ...
    public static List<Lamp> getFromXML(File file) throws SAXException, ParserConfigurationException, IOException {
    ...
    public static Map<String, String> get(List<Lamp>data,String name){
    	Map<String, String>map=new TreeMap<>();
    	OptionalDouble min=data.stream().filter(p->p.getOrg().equals(name)).mapToDouble(e->e.getPrice()).min();
    	OptionalDouble max=data.stream().filter(p->p.getOrg().equals(name)).mapToDouble(e->e.getPrice()).max();
    	double minD;
    	double maxD;
    	if(!min.equals(OptionalDouble.empty()))
    		minD=min.getAsDouble();
    	else minD=0;
    	if(!max.equals(OptionalDouble.empty()))
    		maxD=max.getAsDouble();
    	else maxD=0;
    	String s="from "+minD+" to "+maxD;
    	map.put(name,s);
    	return map;
    }
}
